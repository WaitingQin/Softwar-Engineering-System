...
#include <conio.h>
#include <windows.h>
#include<mmsystem.h>//调用音乐
#pragma comment(lib,"Winmm.lib")//调用音乐
...
void searchVIP(struct VIP **library,int a);//查找会员信息
...
{
    int i;
    char passwords1[100]={0},passwords2[100]={0};//不同功用的密码
    ...
    while(1)
    {
        ...
        for(i=0;i<100;i++)
        {
            passwords1[i]=getch();//录入第一个密码但不显示
...
void addVIP(struct VIP **library)//插入新的会员信息
{
    ...
    while(1)
    {
        ...
        printf("按0键继续\n\n按任意键返回主菜单\n");
        ch=getch();
        if(ch!='0')//根据用户输入确定是否需要继续大while循环
        {
            break;
        }
        else
...
void printLibrary(struct VIP *library)//打印会员信息（全部）
{
    ...
    system("pause");//系统函数，用于暂停，等待用户输入任意键以继续（如果去掉则整个信息会一闪而过）
    system("cls");
}
...
{
    struct VIP *vip;
    char target1[100]={0};//姓名
    char target2[100]={0};//输入的密码
    int target3;//ID号
    int i,j;
    char k;
...
int main()
{
	...
	printf("\n\n\n\n\n\n\n\n\n\n\n\n                                               正在开机。。。。。。               \n\n\n\n\n\n\n\n\n\n\n");//调用音乐，可删
    PlaySound(TEXT("Falcom Sound Team jdk - おやすみ.wav"),NULL,SND_FILENAME | SND_ASYNC | SND_LOOP);//调用音乐，可删
    Sleep(12000);//调用音乐，可删
    PlaySound(NULL,NULL,SND_FILENAME);//调用音乐，可删
    system("cls");//调用音乐，可删
